...
class RatingController extends Controller
{
    ...
    public function checkPendingRatings(Request $request)
    {
        ...
        Log::info('Проверка оценок для сделки', [
            'deal_id' => $dealId,
        ...
        try {
            ...
            if (!in_array($currentUser->status, ['coordinator', 'partner', 'client'])) {
            ...
            foreach ($dealUsers as $user) {
                ...
                elseif ($currentUser->status === 'partner' || $currentUser->status === 'client') {
                    // Партнер и клиент оценивают исполнителей и координатора
                    $shouldRate = in_array($user->status, ['architect', 'designer', 'visualizer', 'coordinator']);
                    $shouldRateReason = 'Партнер/Клиент → Исполнитель/Координатор';
                }
            ...
            return response()->json([
                ...
                'force_rating' => true,
                'user_status' => $currentUser->status,
                // Добавляем дебаг-информацию в ответ
                'debug' => [
                    'deal_id' => $dealId,
                    'deal_status' => $deal->status,
                    'total_users_count' => $dealUsers->count(),
                    'executor_ids' => $executorIds,
                    'current_user' => [
                        'id' => $currentUser->id,
